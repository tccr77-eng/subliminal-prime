...
// Payment: Stripe Payment Element (card, Apple Pay, Google Pay, Klarna, PayPal)
...
// Stripe publishable key — set VITE_STRIPE_PUBLISHABLE_KEY in Vercel environment variables
...
function PaymentForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(errs).length > 0) {
      Object.entries(errs).forEach(([k, v]) => handleChange(k, form[k])); // trigger re-render
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="card-clean p-7">
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-700 text-[#1a1f2e]" style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700 }}>Payment</h3>
        ...
        <PaymentElement
          options={{
            layout: "tabs",
            paymentMethodOrder: ["apple_pay", "google_pay", "card", "klarna", "paypal"],
          }}
        />
        {paymentError && (
          <p className="text-sm text-red-500 mt-3 p-3 bg-red-50 rounded-lg">{paymentError}</p>
        )}
      </div>
...
// ── Fallback form (no Stripe key configured) ─────────────────────────────────
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="card-clean p-7">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-1.5 text-xs text-[#6b7280]"><Lock size={12} /> Secured by Stripe</div>
        </div>
      ...
      <button type="submit" disabled={loading} className="btn-primary w-full justify-center text-base py-4 disabled:opacity-60 disabled:cursor-not-allowed">
        {loading ? (
          <span className="flex items-center gap-2">
            <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
...
export default function Checkout() {
  ...
  // Create PaymentIntent when Stripe is configured
  useEffect(() => {
    if (!stripePromise || items.length === 0) return;
    const amountPence = Math.round(subtotal * 100);
      ...
      <div className="max-w-[1320px] mx-auto px-5 sm:px-8 lg:px-12 py-12 lg:py-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
          <div className="lg:col-span-3">
            ...
            {/* Use Stripe Payment Element if configured, otherwise fallback */}
